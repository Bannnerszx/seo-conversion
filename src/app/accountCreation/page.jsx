...
export default async function AccountCreation() {
  // 1️⃣ Read comma-separated list of legacy cookie names and the current cookie name
  ...
  for (const legacy of oldNames) {
    if (cookieStore.get(legacy)?.value) {
      redirect("/login")
    }
  }
  ...
  try {
    ...
    console.error("[AccountCreation] /api/verify-session error:", err)
    redirect("/login")
  }

  // 8️⃣ Fetch any needed data

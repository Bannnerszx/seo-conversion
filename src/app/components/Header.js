...
const MitsubishiIcon = () => (
    <svg className="w-6 h-6" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path fill={'red'} d="M8 22.38H0l4-6.92h8zm8 0h8l-4-6.92h-8zm0-13.84l-4-6.92-4 6.92 4 6.92z" />
    </svg>
)
const categoryIcons = {
    Toyota: ToyotaIcon,
                        ...
                        >
                            ...
                            >
                                <div className="w-56 bg-white border border-gray-200 rounded-md shadow-lg">
                                    <div className="p-2">
                                        <Link href={`/stock/recommended`} className="flex items-center gap-2 p-2 hover:bg-gray-50 rounded cursor-pointer">
                                            <TrendingUp className="h-4 w-4" />
                                            <span className="font-medium">Recommended</span>
                                        </Link>
                                        <Link href={`/stock/recommended/sale`} className="flex items-center gap-2 p-2 hover:bg-gray-50 rounded cursor-pointer">
                                            <Tag className="h-4 w-4" />
                                            <span className="font-medium">Sale</span>
                                        </Link>
                                        <Link href={`/stock`} className="flex items-center gap-2 p-2 hover:bg-gray-50 rounded cursor-pointer">
                                    ...
                                    <div className="p-2">
                                        {categories.map((category, idx) => {
                                            ...
                                            return (
                                                <Link key={idx} href={`/stock/${category}`} className="flex items-center p-2 hover:bg-gray-50 rounded cursor-pointer">
...
function DesktopAuth() {
    ...
    if (loading) {
        return (
            <div className="flex items-center gap-4">
                {/* responsive skeleton: 
                   - Mobile/Tablet: w-15 h-15 rounded-lg (Matches the square icon buttons)
                   - Desktop (1117px+): w-36 h-12 rounded-sm (Matches the wide buttons)
                */}
                <Skeleton className="w-[45px] h-[45px] rounded-lg min-[1117px]:w-36 min-[1117px]:h-12 min-[1117px]:rounded-sm" />
                <Skeleton className="w-[45px] h-[45px] rounded-lg min-[1117px]:w-36 min-[1117px]:h-12 min-[1117px]:rounded-sm" />
            </div>
        )
    }
    if (user) {
            ...
            <div className="flex items-center gap-4 font-semibold z-[9999]">
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <button className="relative w-15 h-15 flex flex-col items-center justify-center rounded-lg transition group min-[1117px]:flex-row min-[1117px]:gap-2 min-[1117px]:px-4 min-[1117px]:py-2 min-[1117px]:bg-transparent min-[1117px]:rounded-sm z-[9999]">
                            <div className="relative">
                                <CircleUser className="w-7 h-7 min-[1117px]:text-[#0000ff] min-[320px]:text-[#0000ff] z-[9999] transform transition-transform duration-200 ease-in-out group-hover:-translate-y-1" />
                                <span className="absolute -top-2 -right-1 inline-flex items-center justify-center w-5 h-5 text-xs font-semibold text-white bg-red-500 rounded-full transform transition-transform duration-200 ease-in-out group-hover:-translate-y-1">
                    ...
                    <DropdownMenuContent align="end" className="w-56 z-[9999]">
                        ...
                        <DropdownMenuItem onClick={() => logOut()} className="flex items-center gap-2 cursor-pointer text-red-600 hover:text-red-700">
        ...
        return (
            <div className="flex items-center gap-4">
                <Link href="/signup" className="w-15 h-15 flex flex-col items-center justify-center rounded-lg transition group min-[1117px]:flex-row min-[1117px]:gap-2 min-[1117px]:px-4 min-[1117px]:py-2 min-[1117px]:border min-[1117px]:border-blue-600 min-[1117px]:text-blue-600 min-[1117px]:rounded-sm">
                    <UserPlus className="w-6 h- text-[#0000ff] group-hover:text-[#0036b1]" />
                    <span className="whitespace-nowrap text-sm font-semibold text-center text-gray-700 group-hover:text-[#a0a0a0] min-[1117px]:text-[#0000ff]">Sign Up</span>
                </Link>
                <Link href="/login" className="whitespace-nowrap w-15 h-15 flex flex-col items-center justify-center rounded-lg transition group min-[1117px]:flex-row min-[1117px]:gap-2 min-[1117px]:px-4 min-[1117px]:py-2 min-[1117px]:bg-[#0000ff] min-[1117px]:text-white min-[1117px]:rounded-sm">
        ...
        <div className="lg:hidden">
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
                <SheetTrigger asChild>
                    <button onClick={() => setIsOpen(!isOpen)} className="w-15 h-15 flex flex-col items-center justify-center border-gray-300 rounded-lg transition group">
                            ...
                            <div className="items-center space-x-2 mx-auto">
                                ...
                                <Link href="https://www.jumvea.or.jp/jpn/members/Yanagisawa-706" target="_blank" className="block mt-4">
                                    <Image
                                        ...
                                        height={45} // ✅ OPTIMIZATION: Matches max-h-[35px]
                                        quality={75}
                                        sizes="250px"
                                        className="max-h-[45px] w-auto mx-auto"
                                    />
...
export default function Header({ currency, counts, headerRef, showBanner, setShowBanner }) {
    ...
    if (pathname === "/") {
        return (
            <div ref={headerRef} className="z-[7500]">
                ...
                <header className={`${isFixed ? "fixed top-0 z-[50] bg-white/80" : "z-[50]"} shadow-md backdrop-blur-lg h-[75px] w-full  border-solid border-b-4 border-[#0000ff]`}>
                    <div className="flex items-center justify-between h-full z-[9191]">
                        ...
                        <div className="flex items-center space-x-5 mr-5">
                            <div className="hidden lg:flex lg:flex-col items-center justify-center h-full space-y-1">
                                <Link href="https://www.jumvea.or.jp/jpn/members/Yanagisawa-706" target="_blank" className="overflow-hidden">
                                    <ImageHeader
                                        src={jumvea}
                                        alt="REAL MOTOR JAPAN"
                                        width={72}  // ✅ OPTIMIZATION: Scaled to match height of 35px
                                ...
                                <p className="text-sm font-bold text-gray-700 text-center">Proud member</p>
                            </div>
    ...
    return (
        <div className="z-[5000]">
            <header className="fixed top-0 left-0 right-0 z-50 h-[75px] w-full bg-white/80 shadow-md border-b-4 border-[#0000ff]">
